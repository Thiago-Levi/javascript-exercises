...
const namesOrdered = names.map((item) => item).sort();
...
const charactersOrdered = characters.map((item) => {
  return { id: item.id, name: item.name };
});
charactersOrdered.sort((frstId, secondId) => frstId.id - secondId.id);
...
const numberOver10 = randomNumbers.find((number) => number > 50);
console.log(numberOver10);
...
const peoples = ["Cauã", "Alfredo", "Bruno"];
const peopleCopy = peoples.map((people) => people);
...
const concatIngredients = ingredients.reduce(
  (accumulator, item, index, array) => {
    const isFemaleOrMale = /a$/.test(item) ? "cozida" : "cozido";
    const lastItemArray = index === array.length - 1;
    if (lastItemArray) {
      return accumulator + `${item} ${isFemaleOrMale}`;
    }
    return accumulator + `${item} ${isFemaleOrMale}, `;
  },
  ""
...
listGroup.innerHTML = movieNames;
