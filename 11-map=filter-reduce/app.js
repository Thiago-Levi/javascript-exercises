...
const countLess501 = (accumulator, number) =>
  number < 501 ? ++accumulator : accumulator;
const numbersLess501 = crazyNumbers.reduce(countLess501, 0);
...
const getSquaredResult = numbers.map(getSquared);
console.log(getSquaredResult);
/*
  04
...
const showNameCart = cart.reduce(
  (accumulator, { name }) => `${accumulator}- ${name} \n`,
  ""
);

console.log(showNameCart);
...
const nametvShows = tvShows.map(({ name }) => {
  return name;
});
