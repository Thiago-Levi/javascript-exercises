...
const getPokemon = (url, callback) => {
  ...
  request.addEventListener("readystatechange", () => {
    const isRequestOk = request.readyState === 4 && request.status;
...
getPokemon(getPokemonName("squirtle"), (error, data) => {
  logPokemonData(error, data);
  getPokemon(getPokemonName("bulbasaur"), (error, data) => {
    logPokemonData(error, data);
  });
});
...
console.log(map(arraytest, (number) => number * 3));
...
testeForConvertToHex.forEach((color) => console.log(convertToHex(color)));
